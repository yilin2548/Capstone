  ...
  function TypeAuthzController($scope, TypesAuthz) {
    ...
    function authzUserItem(item, user) {
      ...
      if (item && item.$promise) {
        ...
        vm.authz.canUpdateThing = false;
        vm.authz.canRemoveThing = false;      
    ...
    function checkAccess(item) {
      ...
      vm.authz.canUpdateThing = TypesAuthz.canUpdateThing(item);
      vm.authz.canRemoveThing = TypesAuthz.canRemoveThing(item);      
